...
class App {
  ...
  constructor() {
    ...
    containerWorkouts.addEventListener('click', this._moveToPopuap.bind(this));
  }
  ...
  _loadMap(position) {
    ...
    this.#map = L.map('map').setView(coords, this.#mapZoomLeval);
  ...
  _newNetwork(e) {
    e.preventDefault();
    const isValid = (...inputs) => inputs.every(inp => isFinite(inp));
  ...
  _moveToPopuap(e) {
    ...
    this.#map.setView(workout.coords, this.#mapZoomLeval, {
      animate: true,
      pan: {
        duration: 1,
      },
